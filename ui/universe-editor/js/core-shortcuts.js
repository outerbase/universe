    ...
    _detectKeys(e) {
        if (e.key === 'Tab') {
            ...
            this.parent.dispatchEvent(
                new CustomEvent('universe-event', { bubbles: true, composed: true, detail: { execute: true, code: this.editor.value } })
            )
        } else if (e.key === 'Enter') {
            e.preventDefault()
            this._newLine(e)
        ...
        setTimeout(() => {
            this.parent.dispatchEvent(
                new CustomEvent('universe-event', { bubbles: true, composed: true, detail: { code: this.editor.value } })
            )
        }, 50)
        ...
        this.parent.broadcastEvent(this, 'attributeChangedCallback', {
            name: 'code',
            oldValue: this.editor.value,
    ...
    _indentLine(direction) {
        ...
        if (direction === 'right') {
            ...
            if (lineIndent.startsWith(tab)) {
                this.editor.value =
                    beforeText.substring(0, lineStart) + beforeText.substring(lineStart + tabLength) + selectedText + afterText
                // Adjust the cursor position
